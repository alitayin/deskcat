...
- Sprite import scripts only do deterministic work: `4x4` grid slicing, green-screen removal, contact sheet output, and non-polluting staging. They are not the final visual judge.
...
- Do not hand-write frame names for normal imports. Use the action importer so unused `4x4` cells are consistently named `_skip-*` and only the selected action is replaced. If the best frames are not the first cells, pass a comma-separated cell list in playback order.
- Before committing sprite assets, an agent must inspect the generated contact sheet and the individual PNG frames. Reject the set only if visual review shows obvious placement jumps, size changes, wrong direction, cropped body parts, ghosting, visible green-screen leftovers, or illogical/non-continuous motion.
