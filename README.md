...
动画素材可以直接放入 `assets/pet`，按 `动作名-数字.png` 命名，例如 `daze-1.png`、`look-1.png`、`lazy-1.png`、`walk-left-1.png`。运行时会按数字顺序播放。

运行时显示尺寸和鼠标命中区域：散步是 `200 x 200`，其他动作是 `160 x 160`。播放速度：散步是 `300ms/帧`，发呆和观察是 `800ms/帧`，其他动作是 `500ms/帧`。
